...
class User extends CI_Controller
{
    ...
    public function submit_jawaban($id){
        ...
        //     $this->load->view('template/footer');
        // } else {
        $generate_jwb = 'jwb' . date('Ymdhis');
        foreach ($_POST['id_quiz'] as $quizId => $value) {
           
            $where = array('id' => $value);
            $checkjawaban = $this->m_quiz->detail_quiz($where);
            $jawabanSiswa = isset($_POST['jawaban'][$quizId]) ? $_POST['jawaban'][$quizId] : 0;
            
            
            if(isset($_POST['jawaban'][$quizId])){
...
//        $this->session->set_flashdata('success-reg', 'Berhasil!');
        ...
        $redirectUrl = base_url('user/detail_jawaban').'/'.$id.'/'.$siswa['id'].'/'.$generate_jwb;
    ...
    //detail_jawaban
    public function detail_jawaban($id, $id_siswa,$generate_jwb) {

        $this->load->model('m_jawaban');
        $where = array('jawaban.id_siswa' => $id_siswa,'quiz.id_materi'=>$id,
        'jawaban.generate_jwb'=>$generate_jwb);
    ...
    public function quiz_kelas($kelas){
       $this->load->model('m_quiz');
        $where = array('quiz.kelas' => $kelas);
        $data['quiz'] = $this->m_quiz->detail_kelas($where, 'quiz');
         $data['user'] = $this->db->get_where('siswa', ['email' =>
            $this->session->userdata('email')])->row_array();
        $this->load->view('user/detail_kelas', $data);
        $this->load->view('template/footer');
    }
